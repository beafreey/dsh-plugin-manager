...
import { PluginManagerPanel } from './Panel.tsx'

/** The injected panel container (kept in the DOM, hidden when inactive). */
export const PANEL_VIEW_SELECTOR = '[data-dsh-pluginmanager-view]'
 ...
 */
export function mountPanel(controller: PluginManagerController): () => void {
  ...
  const ensure = (): void => {
    ...
    root.render(<PluginManagerPanel controller={controller} />)
  }
  ...
  const onOtherActivate = (event: Event): void => {
    if ((event as CustomEvent).detail !== PANEL_NAME && controller.getSnapshot().panelOpen) {
      controller.close()
    }
  }
