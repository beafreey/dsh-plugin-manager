...
var PluginManagerApiError = class extends Error {
	constructor(message) {
		super(message);
...
async function readJson(response) {
	...
	if (!response.ok) throw new PluginManagerApiError(typeof body === "object" && body !== null && typeof body.error === "string" ? body.error : `HTTP ${response.status}`);
	return body;
}
/** The browser half's only data entry point. */
var PluginManagerApi = class {
	/** Profile views (summary + plugin rows) for every managed profile. */
	async list(profile) {
		const query = profile !== void 0 && profile !== "" ? `?profile=${encodeURIComponent(profile)}` : "";
		return readJson(await fetch(PLUGIN_MANAGER_API.list + query)).then((body) => body.profiles);
	}
	/** Run update checks (every profile, or the named one / subset of plugins). */
	async check(profile, names) {
		const body = {};
		if (profile !== void 0 && profile !== "") body.profile = profile;
		if (names !== void 0 && names.length > 0) body.names = names;
		return readJson(await fetch(PLUGIN_MANAGER_API.check, {
			method: "POST",
	...
	async update(profile, name) {
		return (await readJson(await fetch(PLUGIN_MANAGER_API.update, {
			method: "POST",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({
				profile,
	...
	async updateAll(profile) {
		return (await readJson(await fetch(PLUGIN_MANAGER_API.updateAll, {
			method: "POST",
			headers: { "content-type": "application/json" },
	...
	async remove(profile, name) {
		return (await readJson(await fetch(PLUGIN_MANAGER_API.remove, {
			method: "POST",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({
				profile,
...
var PluginManagerController = class {
	...
	*/
	...
	mergeViews(views) {
		...
		for (const view of views) if (!merged.some((existing) => existing.profile.name === view.profile.name)) merged.push(view);
		const active = this.snapshot.active !== "" && merged.some((view) => view.profile.name === this.snapshot.active) ? this.snapshot.active : merged[0]?.profile.name ?? "";
		this.emit({
			profiles: merged,
			active
		});
	...
	*/
	async load(forceCheck) {
		...
		if (forceCheck || this.snapshot.profiles.every((view) => view.plugins.every((entry) => entry.state === "unknown"))) await this.check();
	}
	...
	async update(profile, name) {
		...
		if (updating.has(key)) return void 0;
		updating.add(key);
		this.emit({
			updating,
			banner: void 0,
		...
		try {
			...
			this.emit({
				banner: ok ? {
					kind: "ok",
					text: `已更新 ${profile}/${name}（${result.durationMs < 1e3 ? "不足 1 秒" : `${Math.round(result.durationMs / 1e3)} 秒`}）。重启 DSH 后生效。`
				} : {
					kind: "error",
					text: `更新 ${profile}/${name} 失败：${result.error ?? "未知错误"}`
				},
	...
	async updateAll(profile) {
		...
		const targets = this.snapshot.profiles.find((entry) => entry.profile.name === profile)?.plugins.filter((entry) => entry.state === "outdated").map((entry) => entry.name) ?? [];
		if (targets.length === 0) {
			this.emit({ banner: {
				kind: "info",
				text: `profile ${profile} 没有可更新的插件。`
			} });
			return;
		}
		const outputs = [];
		...
		for (const name of targets) {
			...
			if (result?.output !== void 0 && result.output !== "") outputs.push(`$ pnpm → ${profile}/${name}\n${result.output}`);
		}
		if (anyOk) this.emit({
			banner: {
				kind: "ok",
				text: `profile ${profile} 批量更新完成。重启 DSH 后新代码生效。`
			},
			needsRestart: true,
	...
	async remove(profile, name) {
		...
		try {
			...
			this.emit({
				banner: ok ? {
					kind: "ok",
					text: `已从 ${profile} 删除 ${name}。重启 DSH 后完全卸载。`
				} : {
					kind: "error",
					text: `删除 ${profile}/${name} 失败：${result.error ?? "未知错误"}`
				},
				needsRestart: ok || this.snapshot.needsRestart,
				lastUpdateOutput: result.output
			});
...
var PanelErrorBoundary = class extends import_react.Component {
	state = { error: void 0 };
	static getDerivedStateFromError(error) {
		return { error };
	}
	render() {
		if (this.state.error !== void 0) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "dshpm-panel",
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "dshpm-banner",
				"data-kind": "error",
				children: ["面板渲染失败：", this.state.error.message]
			})
		});
...
function PluginRow(props) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
		/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("td", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "dshpm-mono dshpm-strong",
			children: entry.name
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "dshpm-muted dshpm-tiny",
			children: [entry.spec, local ? "（本地 link 安装）" : ""]
		})] }),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", {
			className: "dshpm-mono",
			children: entry.version
		}),
		/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("td", {
			className: "dshpm-mono",
			children: [entry.state === "checking" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "dshpm-spinner" }) : entry.latest ?? "—", entry.latestSource === "git" && entry.latest !== void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "dshpm-muted dshpm-tiny",
				children: " git"
			}) : null]
		}),
		/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("td", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			className: "dshpm-badge",
			"data-kind": meta.kind,
			children: meta.label
		}), entry.error !== void 0 && entry.error !== "" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "dshpm-muted dshpm-tiny dshpm-error-text",
			children: entry.error
		}) : null] }),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: entry.repository !== void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("a", {
			className: "dshpm-link",
			href: entry.repository,
			target: "_blank",
			rel: "noreferrer noopener",
			title: entry.repository,
			children: repoLabel(entry.repository)
		}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			className: "dshpm-muted",
			children: "—"
		}) }),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: updating ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
			className: "dshpm-inline dshpm-muted",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "dshpm-spinner" }), "处理中…"]
		}) : confirming ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
			className: "dshpm-actions",
...
function PluginManagerPanel(props) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(PanelErrorBoundary, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "dshpm-panel",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "dshpm-panelHeader",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
						className: "dshpm-panelTitle",
						children: "插件管理"
					}),
					activeSummary !== void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "dshpm-badge",
						"data-kind": "info",
						title: activeSummary.dir,
						children: activeSummary.name
					}) : null,
					activeSummary?.pnpm !== void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "dshpm-badge",
						"data-kind": "key",
						title: activeSummary.pnpm,
						children: "pnpm ✓"
					}) : activeSummary !== void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "dshpm-badge",
						"data-kind": "fail",
			...
			snapshot.profiles.length > 1 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "dshpm-tabBar",
				children: snapshot.profiles.map((view) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
					type: "button",
					className: "dshpm-tab",
					"data-active": view.profile.name === active ? "true" : void 0,
					onClick: () => {
						controller.setActive(view.profile.name);
					},
					children: [view.profile.name, /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "dshpm-tabCount",
						"data-outdated": view.plugins.some((entry) => entry.state === "outdated") ? "true" : void 0,
						children: view.plugins.length
					})]
				}, view.profile.name))
			}) : null,
			snapshot.needsRestart ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "dshpm-banner",
				"data-kind": "info",
				children: [
					"已更新插件。host 端新代码需要 ",
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: "重启 DSH" }),
					"（完全退出后重新打开）才会加载；浏览器侧刷新页面即可加载新的 client 代码。"
				]
			}) : null,
			snapshot.banner !== void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "dshpm-banner",
				"data-kind": snapshot.banner.kind,
				children: snapshot.banner.text
			}) : null,
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "dshpm-toolbar",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
						type: "button",
						...
						disabled: busy,
						onClick: () => {
							controller.check(active || void 0);
						},
						children: snapshot.checking ? "检查中…" : "检查更新"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
						...
						onClick: () => {
							if (active !== "") controller.updateAll(active);
						},
						children: ["全部更新", outdatedCount > 0 ? `（${outdatedCount}）` : ""]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
						type: "button",
						className: "dshpm-ghostButton",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "dshpm-toolbarSpacer" }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						className: "dshpm-muted dshpm-tiny",
						children: [
							...
							plugins.length,
							" 个第三方插件 · 检查源：",
							activeSummary?.registry ?? "npm registry",
							" + git"
						]
					})
				]
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "dshpm-tableWrap",
				children: snapshot.loading && snapshot.profiles.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "dshpm-loading",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "dshpm-spinner" }), " 正在读取 profile 插件列表…"]
				}) : snapshot.profiles.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "dshpm-empty",
					children: "没有检测到可管理的 dsh profile。"
				}) : plugins.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "dshpm-empty",
					children: [
						"profile「",
						active,
					...
					className: "dshpm-table",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "插件" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "当前版本" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "最新版本" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "状态" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "仓库" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "操作" })
					] }) }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: plugins.map((entry) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(PluginRow, {
						entry,
						updating: snapshot.updating.has(`${active}\u0000${entry.name}`),
						busy,
						onUpdate: (name) => {
							if (active !== "") controller.update(active, name);
						},
						onRemove: (name) => {
							if (active !== "") controller.remove(active, name);
						}
					}, entry.name)) })]
				})
			}),
			snapshot.lastUpdateOutput !== "" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("details", {
				className: "dshpm-details",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("summary", { children: "最近一次 pnpm 更新输出" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("pre", {
					className: "dshpm-pre",
					children: snapshot.lastUpdateOutput
				})]
			}) : null
		]
	}) });
...
const ACTIVE_ATTR = "data-dsh-pluginmanager-active";
...
const PANEL_NAME = "plugin-manager";
...
function mountPanel(controller) {
	...
	const ensure = () => {
		...
		container.dataset.dshPluginmanagerView = "";
		container.className = "dshpm-view";
		column.appendChild(container);
		root = (0, import_client.createRoot)(container);
		root.render(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PluginManagerPanel, { controller }));
	};
...
function createEntry(controller) {
	...
	entry.innerHTML = "<span class=\"dshpm-entryIcon\"><svg viewBox=\"0 0 16 16\" width=\"14\" height=\"14\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.3\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><path d=\"M4.5 2.5h4l3 3v8h-7z\"/><path d=\"M8.5 2.5v3h3\"/><path d=\"M6.5 8h4\"/><path d=\"M6.5 10.5h4\"/></svg></span><span class=\"dshpm-entryLabel\">插件管理</span>";
...
function placeEntry(root, entry) {
	...
	if (entry.parentElement !== root) {
		...
		const family = Array.from(root.children).filter((el) => el instanceof HTMLElement && el.matches("[data-dsh-taskboard-entry], [data-dsh-ssh-entry], [data-dsh-pluginmanager-entry]"));
