...
function findPnpm(explicit, env = process.env) {
	if (explicit !== void 0 && explicit !== "") {
		if (existsSync(explicit)) return resolve(explicit);
		return;
	}
	const candidates = [
		...(env.PATH ?? "").split(":").filter((entry) => entry !== "").map((dir) => join(dir, "pnpm")),
...
function widenedPath(base = process.env) {
	const extra = [
		"/opt/homebrew/bin",
		"/usr/local/bin",
		"/usr/bin",
	...
	for (const dir of extra) if (!seen.has(dir) && existsSync(dir)) seen.add(dir);
	return [...seen].join(":");
}
...
var PluginManager = class {
	...
	resolvePackageDir(name) {
		try {
			const manifestPath = createRequire(join(this.profileDir(), "package.json")).resolve(`${name}/package.json`);
	...
	*/
	...
	async updatePlugin(name) {
		if (this.updating !== void 0) throw new Error(`another profile operation is already running (${this.updating})`);
		const spec = this.readProfileManifest()?.dependencies[name];
		if (spec === void 0) throw new Error(`package '${name}' is not a profile dependency`);
		this.updating = name;
	...
	reconcileBundlesAfterRemoval(name) {
		const manifestPath = join(this.profileDir(), "package.json");
	...
	async removePlugin(name) {
		if (this.updating !== void 0) throw new Error(`another profile operation is already running (${this.updating})`);
		if (this.readProfileManifest()?.dependencies[name] === void 0) throw new Error(`package '${name}' is not a profile dependency`);
		const pnpm = this.pnpmPath();
		if (pnpm === void 0) throw new Error("pnpm not found — install pnpm or set the pnpm path in the plugin settings");
	...
	return [
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				const request = names === void 0 ? {} : { names };
				try {
					writeJson(res, 200, {
						profile: manager.summary(),
						plugins: await manager.checkUpdates(request)
					});
				} catch (error) {
...
function pluginCheckTool(manager) {
	return defineTool({
		...
		description: "List the third-party plugins installed in the active dsh profile with their versions and git repositories, and optionally check the npm registry / git remotes for newer versions. Triggers: plugin update status, check for plugin updates, list installed dsh plugins.",
		parameters: {
			check: {
				type: "boolean",
		...
		output: {
			...
			render: (_args, value) => {
				...
				return text([
					...
					"--- | --- | --- | --- | --- | ---",
					...plugins.map((row) => renderRow(row))
				].join("\n"));
			}
		},
		async execute(args) {
			return { plugins: args.check ?? true ? await manager.checkUpdates(args.names !== void 0 ? { names: args.names } : void 0) : manager.listPlugins() };
		}
	});
}
...
function pluginUpdateTool(manager) {
	return defineTool({
		...
		async execute(args) {
			return { results: args.name !== void 0 && args.name !== "" ? [await manager.updatePlugin(args.name)] : await manager.updateAll() };
		}
	});
}
...
function pluginRemoveTool(manager) {
	return defineTool({
		...
		description: "Remove one installed third-party dsh plugin from the active profile by running pnpm in the profile directory. Host-side plugin code unloads only after dsh restarts. Requires pnpm on the host. Triggers: uninstall a plugin, remove a plugin, delete a dsh plugin.",
		parameters: { name: {
			type: "string",
			required: true,
...
//#endregion
export { Config, PLUGIN_MANAGER_SETTINGS_NAMESPACE, PluginManager, apply, classifySpec, inject, name, specToRepositoryUrl };
